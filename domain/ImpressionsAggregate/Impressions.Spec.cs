...
{
    public class ImpressionsSpec : Impressions
    {
        ...
        {
            return Substitute.ForPartsOf<ImpressionRemovedEvent>(this.Id, objectType);
        }

        public class AddImpressionSpec : ImpressionsSpec
        {
            [It(nameof(Impressions.AddImpression))]
            public void Should_Release_correct_events_when_AddImpression_is_called()
            {
                ...
                events.First().Should().BeAssignableTo<ImpressionAddedEvent>();
                events.Last().Should().BeAssignableTo<ImpressionsChangedEvent>();
            }
        ...
        {
            [It(nameof(Impressions.RemoveImpression))]
            public void Should_Release_correct_events_when_RemoveImpression_is_called()
            {
                ...
                events.Should().HaveCount(2);
                events.First().Should().BeAssignableTo<ImpressionRemovedEvent>();
                events.Last().Should().BeAssignableTo<ImpressionsChangedEvent>();
            }
